...
fig2 <-ggplot(fig2_data, aes(y=weighted, x=local_authority, 
  ...
  labs(y = "Co-Benefits per Capita (GBP)",
    title = "Per Capita Gain for the 5 strongest Co-Benefits in the Top and Bottom Beneficiaries, 2025 to 2050")
...
# make count
mosaic <- mosaic %>% uncount(weighted, .remove = TRUE, .id = NULL)

# factorise and rename observations
mosaic$divider <- factor(mosaic$divider,
                         levels = c("Top 3 Local Authorities", "Bottom 3 Local Authorities"))

mosaic$local_authority <- gsub("East Renfrewshire", "East\nRenfrewshire", mosaic$local_authority)
mosaic$local_authority <- gsub("East Dunbartonshire", "East\nDunbartonshire", mosaic$local_authority)

mosaic$co_benefit <- gsub("activity_weighted", "Physical Activity", mosaic$co_benefit)
mosaic$co_benefit <- gsub("air_weighted", "Air Quality", mosaic$co_benefit)
mosaic$co_benefit <- gsub("noise_weighted", "Noise", mosaic$co_benefit)
mosaic$co_benefit <- gsub("cold_weighted", "Excess Cold", mosaic$co_benefit)
mosaic$co_benefit <- gsub("diet_weighted", "Diet Change", mosaic$co_benefit)

mosaic$co_benefit <- factor(mosaic$co_benefit,
                               levels=c("Physical Activity", "Air Quality",
                                        "Noise", "Excess Cold",
                                        "Diet Change"))
...
ggplot(mosaic) +
  ...
  #geom_mosaic_text(aes(x=product(co_benefit, local_authority), fill = co_benefit))+
  ...
  scale_fill_manual(labels = c("Diet Change", "Reduced Excess Cold", 
                     ...
                     values = c("#31688e", "#21918c", "#35b779", "#ed4a89", "#fbb61a"))+
  labs(title = "The Composition Gap: Why Cities Lag", 
       subtitle = "Five Highest Co-Benefits per Capita in the Top and Bottom Beneficiaries, 2025 to 2050", 
       caption = "The disparity in potential is driven by baseline behaviors. High-gain areas benefit from massive shifts away from car\ndependency, translating into high scores for physical activity and air quality. In cities where public transport use is already\nhigher, the 'delta' for improvement is smaller, making the transition high-effort but relatively lower-reward on a per-person basis.")
